...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase.from("vw_daily_station_summary")
        .select("*").gte("transaction_date", from).lte("transaction_date", to)
  ...
  const byStation = summary.reduce((acc, row) => {
    if (!acc[row.station_id]) acc[row.station_id] = {
      station_name: row.station_name, total_revenue: 0, total_litres: 0,
      cash: 0, momo: 0, credit: 0, lpo: 0,
    };
    ...
    acc[row.station_id].momo += row.momo_revenue + row.airtel_revenue;
    acc[row.station_id].credit += row.credit_revenue;
    acc[row.station_id].lpo += row.lpo_revenue;
    return acc;
  }, {} as Record<string, any>);

  const grandRevenue = Object.values(byStation).reduce((s: number, r: any) => s + r.total_revenue, 0);
  const grandLitres = Object.values(byStation).reduce((s: number, r: any) => s + r.total_litres, 0);
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Stations Reporting", value: Object.keys(byStation).length, color: "text-purple-700" },
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="data-table">
                <thead>
                  <tr><th>Station</th><th className="text-right">Litres</th><th className="text-right">Revenue</th><th className="text-right">Cash</th><th className="text-right">MoMo</th><th className="text-right">Credit</th><th className="text-right">LPO</th></tr>
                </thead>
                <tbody>
        ...
        <div className="card overflow-hidden">
          ...
          <div className="overflow-x-auto">
            {loading ? <div className="p-10 text-center text-gray-400 text-sm">Loading...</div> : (
              <table className="data-table">
                <thead>
                  <tr><th>Date</th><th>Station</th><th className="text-right">Litres</th><th className="text-right">Revenue</th><th className="text-right">Cash</th><th className="text-right">MoMo</th><th className="text-right">Credit</th></tr>
                </thead>
                <tbody>
                  {summary.map((row) => (
                    <tr key={`${row.station_id}-${row.transaction_date}`}>
                      ...
                      <td className="text-right">{formatUGX(row.momo_revenue + row.airtel_revenue)}</td>
