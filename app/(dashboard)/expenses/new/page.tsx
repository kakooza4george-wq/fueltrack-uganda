...
interface ExpenseForm {
  station_id: string; expense_date: string; category: string;
  sub_description: string; amount_ugx: string; payment_method: string;
  payee: string; receipt_reference: string;
  entered_by: string; approved_by: string; notes: string;
}
...
export default function NewExpensePage() {
  ...
  const { register, handleSubmit, setValue } = useForm<ExpenseForm>({
    defaultValues: { station_id: activeStation?.id ?? "", expense_date: today(), payment_method: "cash" },
  });
  ...
  const onSubmit = async (data: ExpenseForm) => {
    ...
    const { error: err } = await supabase.from("expenses").insert({
      station_id: data.station_id, expense_date: data.expense_date,
      category: data.category, sub_description: data.sub_description || null,
      amount_ugx: parseFloat(data.amount_ugx), payment_method: data.payment_method as any || null,
      payee: data.payee || null, receipt_reference: data.receipt_reference || null,
      entered_by: data.entered_by || null, approved_by: data.approved_by || null,
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <input type="text" className="form-input" placeholder="More detail about this expense"
              ...
              <div>
                ...
                <input type="text" className="form-input" {...register("payee")} />
              </div>
              <div>
                <label className="form-label">Receipt / Invoice #</label>
                <input type="text" className="form-input" {...register("receipt_reference")} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" className="form-input" {...register("approved_by")} />
              </div>
              <div className="sm:col-span-2">
